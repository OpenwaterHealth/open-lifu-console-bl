...
static uint8_t  USBD_DFU_EP0_TxReady(USBD_HandleTypeDef *pdev)
{
  ...
  if (hdfu->dev_state == DFU_STATE_DNLOAD_BUSY)
  {
    ...
    if (hdfu->wblock_num == 0U)
    {
      ...
      else if ((hdfu->buffer.d8[0] == DFU_CMD_ERASE) && ((hdfu->wlength == 5U) || (hdfu->wlength == 1U)))
      {
        if (hdfu->wlength == 5U)
        {
          hdfu->data_ptr = hdfu->buffer.d8[1];
        ...
        {
          /* DfuSe mass erase command (0x41 only): media layer interprets 0xFFFFFFFFU as erase-all */
          hdfu->data_ptr = 0xFFFFFFFFU;
        }

        if (((USBD_DFU_MediaTypeDef *)pdev->pUserData)->Erase(hdfu->data_ptr) != USBD_OK)
        {
          hdfu->dev_state = DFU_STATE_ERROR;
          hdfu->dev_status[0] = DFU_ERROR_ERASE;
          hdfu->dev_status[1] = 0U;
          hdfu->dev_status[2] = 0U;
          hdfu->dev_status[3] = 0U;
    ...
    else
    {
      if (hdfu->wblock_num > 1U)
      {
        ...
        {
          hdfu->dev_state = DFU_STATE_ERROR;
          hdfu->dev_status[0] = DFU_ERROR_WRITE;
          hdfu->dev_status[1] = 0U;
          hdfu->dev_status[2] = 0U;
          hdfu->dev_status[3] = 0U;
          hdfu->dev_status[4] = hdfu->dev_state;
