...
/* Production protections available on the STM32F072 (Cortex-M0).
   NOTE: unlike the H743 reference, the F072 has NO MPU, NO PCROP, NO firewall,
   NO secure-user memory and no DAP-disable option byte — enabling any of those
   switches raises a #error in sfu_low_level_security.h. The effective hardware
   protections on this part are WRP (bootloader pages), RDP (level 2 in
   production also disables the debug port) and the IWDG. */

#define SFU_WRP_PROTECT_ENABLE
#define SFU_RDP_PROTECT_ENABLE
/*#define SFU_TAMPER_PROTECT_ENABLE*/ /*!< Not ported: no tamper GPIO wiring on the lifu console board */
#define SFU_DMA_PROTECT_ENABLE
/*#define SFU_IWDG_PROTECT_ENABLE*/  /*!< IWDG is armed and refreshed from main.c in this port */
