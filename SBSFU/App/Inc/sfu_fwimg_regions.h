...
/* Single-image build: the slot address tables are defined ONCE (by
 * sfu_fwimg_common.c). In ST's two-binary reference SE_LOW_LEVEL_C also
 * instantiated them for the separate SECoreBin ELF — that would be a
 * duplicate symbol here. */
#if defined(SFU_FWIMG_COMMON_C)
