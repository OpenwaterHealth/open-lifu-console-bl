...
#include "sfu_error.h"
#include "stm32f0xx_it.h" /* required for the HAL Cube callbacks */
/*
...
{
  /* No BSP LED/GPIO layer in this port: the status LEDs are driven from
   * main.c in DFU mode and the trace UART MSP handles its own pins. */
      ...
      if (SFU_SUCCESS == SFU_IMG_DetectFW(SLOT_ACTIVE_1 + i))
      {
        ...
        if (SFU_SUCCESS == e_ret_status)
        {
          ...
          if (SFU_SUCCESS == e_ret_status)
          {
             ...
             * NOTE: the application's persistent-config flash region MUST live
             * OUTSIDE this slot (see Core/Inc/memory_map.h — LIFU config page
             * 0x0801F000, user config 0x0801F800). If it were inside the slot,
             * the app writing its config would leave non-blank data past the
             * image and this check would fail on the next boot, invalidating a
             * perfectly good firmware. */
...
/**
  ...
  * @retval None
  */
void SFU_CALLBACK_ANTITAMPER(void *hrtc)
{
  /* Tamper protection is not ported to the F072 (no RTC HAL in this build);
   * keep the callback for API compatibility. */
  UNUSED(hrtc);
