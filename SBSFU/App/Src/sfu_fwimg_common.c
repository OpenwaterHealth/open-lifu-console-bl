...
SFU_IMG_InitStatusTypeDef SFU_IMG_InitImageHandling(void)
{
  ...
  /*
   * The H743 reference performs an extra sanity check here ("headers are under
   * the MPU protection"). The STM32F072 (Cortex-M0) has no MPU, so that check
...
SFU_ErrorStatus SFU_IMG_LaunchActiveImg(uint32_t SlotNumber)
{
   ...
   * vector[0]=initial SP, vector[1]=reset handler. */
  {
    ...
    /* Return the clock tree to its reset state (HSI 8 MHz, PLL off) so the
     * application's SystemClock_Config starts from a virgin RCC. If the PLL
     * is left running as SYSCLK, the app's HAL_RCC_OscConfig(PLL_ON) returns
     * HAL_ERROR ("PLL used as system clock") and the app parks in its
     * Error_Handler before printing anything — hardware-observed on this
     * board; the legacy bootloader reset the RCC before jumping for the same
     * reason. */
    (void)HAL_RCC_DeInit();

    /* Stop the SysTick counter completely and clear any pending SysTick/PendSV
     * so nothing vectors into the application table before its runtime is up
     * (the SYSTICK_SUSPEND syscall above only masked the interrupt). */
    SysTick->CTRL = 0U;
    SysTick->VAL  = 0U;
     ...
     * app-privileged model). Done while still privileged, before masking
     * interrupts, and BEFORE the vector-table copy below which reuses the
     * bottom of SRAM the wipe just cleared. */
    ...
    /* Disable and clear ALL peripheral interrupts (single 32-bit bank on
     * Cortex-M0) so nothing pends into the application's vector table before
     * its runtime is initialised — mirrors the proven legacy-bootloader jump. */
    NVIC->ICER[0U] = 0xFFFFFFFFUL;
    NVIC->ICPR[0U] = 0xFFFFFFFFUL;

    /* The Cortex-M0 has no VTOR: relocate the application vector table by
     * copying its 48 entries to the base of SRAM and remapping SRAM at
     * 0x00000000 (SYSCFG_CFGR1 MEM_MODE) — the same mechanism as the legacy
     * lifu bootloader, so applications behave identically after a relink. */
    for (i = 0U; i < 48U; i++)
    {
      ((volatile uint32_t *)SRAM_BASE)[i] = ((const volatile uint32_t *)app_vector)[i];
    }
     ...
     * which only advances from the SysTick interrupt (lesson learned on the H743
     * reference port). */
    __enable_irq();
