...
/* Post-download hooks polled by the bootloader main loop (see main.c).
 * DFU_ImageDownloadComplete() returns 1 once a new image has been programmed
 * AND the host has finished the manifestation phase. */
uint8_t DFU_ImageDownloadComplete(void);

/* Anti-rollback (DFU-time). DFU_IsRollback() returns 1 if the just-downloaded
 * image carries a FwVersion lower than the version installed at DFU entry.
